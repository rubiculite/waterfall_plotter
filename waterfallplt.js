...
function waterfallPlot(d3_AppendToElement,data, size) {
   ...
   if (data == undefined) {
      this.wf = (function () {
         ...
         wf.iLabel = '';
         wf.data = [
            {'x':wf.xMin,'y':wf.yMin,'jy':wf.iMax,'mask':0},
            {'x':(wf.xMax+wf.xMin)/2,'y':wf.yMin,'jy':wf.iMax,'mask':0},
            {'x':wf.xMin,'y':(wf.yMax+wf.yMin)/2,'jy':wf.iMax,'mask':0},
            {'x':(wf.xMax+wf.xMin)/2,'y':(wf.yMax+wf.yMin)/2,'jy':wf.iMax,'mask':0}
         ]
         wf.dataX  = [0,0] 
         wf.dataY  = [0,0] 
         wf.xValue = (function(xMin,xMax,xBins){return function(bin){return Number((xMax-xMin)*bin/xBins+xMin).toFixed(2);};})(wf.xMin,wf.xMax,wf.xBins);
         wf.yValue = (function(yMin,yMax,yBins){return function(bin){return Number((yMax-yMin)*bin/yBins+yMin).toFixed(2);};})(wf.yMin,wf.yMax,wf.yBins);
         return wf;
      })(); 
